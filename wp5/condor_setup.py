...
# data settings
n_samples = "max" #"max" # per category
...
if not isinstance(n_samples, int) and n_samples.lower()=='max':
    print(f'#clean={clean_segments.shape[0]}, #whistle={len(df_w)}, #tomte={len(df_t)}, #scatteredlight={len(df_s)}')
    # maximum possible samples with a balanced dataset
    n_samples = min([clean_segments.shape[0], len(df_w), len(df_t), len(df_s)])
print(f'{n_samples=} per class')
...
np.savez(f'ids_n={n_samples}.npz', gs_idx=gs_idx, times=times, labels=labels)
np.save('all_times.npy', times)
# condor settings
n_jobs = 8
...
request_memory = 5000 # in MB | test on single system first
request_disk = 500 + (timesparallel+maxqueuesize+nproc)*350 + max(len(split_times[i]) for i in range(len(split_times)))*17 # in MB | each temporary file on disk is ~280 MB and each gpstime contributes ~ 17 MB to the results file (mostly the qscan)
...
sh_lines = ['#!/bin/bash', #"echo 'export PATH=/cvmfs/oasis.opensciencegrid.org/ligo/sw/conda/bin/:$PATH'",
            ...
            f'python3 {py_script} -t ${{times}} -w ${{width}} -s ${{segs}} -o ${{overlap}} -p ${{path}} -q ${{maxqueuesize}} -r ${{maxretries}} -n ${{nproc}} -m ${{timesparallel}} --verbose']
...
sub_lines = ['universe = vanilla',
             'getenv = true',
             'environment = ID=$(ID);times=$(times);path=$(path);width=$(width);segs=$(segs);overlap=$(overlap);maxqueuesize=$(maxqueuesize);maxretries=$(maxretries);nproc=$(nproc);timesparallel=$(timesparallel)',
             ...
             'igwn_oauth_permissions = read:/frames',
             'use_x509userproxy = true',
             f'x509userproxy = {proxy_path}',
             'requirements = HAS_LIGO_FRAMES =?= True',  # HAS_CVMFS_IGWN_PRIVATE_DATA =?= True # doesnt seem to work
             '+DESIRED_Sites = "LIGO-CIT,LIGO-LA"', # Only running on CIT and Livingston as they seem to be the only ones with L1_R frames
             'should_transfer_files = YES',
             f'transfer_input_files = {times_dir}/$(times), {py_script}, {channels_file}', 
...
for i in range(1,n_jobs+1):
    ...
    dag_lines.append(f'VARS A{i} ID="{i}" times="J{i}_times.npy" path="{res_dir}" width="{width}" segs="{" ".join(segs)}" overlap="{overlap}" maxqueuesize="{maxqueuesize}" maxretries="{maxretries}" nproc="{nproc}" timesparallel="{timesparallel}"')
    #dag_lines.append(f'RETRY A{i} 3') # for final run
